...
struct BreathingView: View {
    ...
    var body: some View {
        GeometryReader { rootGeo in
            let compact = rootGeo.size.height < 212
            ...
            ScreenScaffold {
                ...
                GeometryReader { geo in
                    let ringSize = min(geo.size.width, geo.size.height) * (compact ? 0.64 : 0.78)

                    ZStack {
                        Circle()
                            .stroke(AppTheme.ColorToken.neutralGray, lineWidth: compact ? 4 : 5)
                        .frame(width: ringSize, height: ringSize)

                        Circle()
                            .stroke(AppTheme.ColorToken.successGreen, lineWidth: compact ? 2.4 : 3)
                        .frame(width: ringSize * 0.78, height: ringSize * 0.78)
                            .scaleEffect(pulse ? (reduceMotion ? 1.02 : 1.06) : 0.95)
                ...
                VStack(spacing: compact ? AppTheme.Spacing.xxs : AppTheme.Spacing.xs) {
